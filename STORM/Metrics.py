...
def run_mclust_ari(ad, key: str, n_clusters: int = None, model="EEE") -> float:
    X = np.asarray(ad.obsm[key], float)
    X = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)

    if n_clusters is None:
        n_clusters = int(ad.obs['layer'].nunique())
    ...
    fit = mclust.Mclust(Xr, G=n_clusters, modelNames=model)

    pred_r = fit.rx2("classification")
    pred = np.array(pred_r, dtype=int).ravel()

    ad.obs['clustered_spatial_domains'] = pred
    obs_df = ad.obs.dropna(subset=['layer'])
    ari = float(adjusted_rand_score(obs_df['clustered_spatial_domains'].loc[obs_df.index],
                                    obs_df['layer']))
    u, c = np.unique(pred, return_counts=True)
    print(f"[R-mclust ARI/{key}] k={n_clusters} uniq={u} counts={c} ARI={ari:.4f}")
    return ari
