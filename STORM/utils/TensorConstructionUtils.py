...
def lin_indices_for_sample(
    ...
    """
    Return 1D grid indices for each spot in `adata`, in the same
    order as adata.obs, for an NX x NY grid.
    """
    rows = adata.obs[row_key].to_numpy().astype(int)
    cols = adata.obs[col_key].to_numpy().astype(int)
    lin  = rows * NY + cols       # linear index into the NX*NY grid
    return lin