...
def _preprocess_and_hvg(adata, n_top_genes=3000):
...
def _gene_intersection(adatas, use_hvg=True, reference=0):
    ...
    # preserve order from reference AnnData
...
def make_synthetic_spots_from_single_cells(
    ...
    bin_size: float = 200.0,          # square bin width/height 
    coord_key: str = "spatial",       # adata_cells.obsm[coord_key][:, :2]
    min_cells_per_spot: int = 2,
    layer: str | None = None,         # e.g. "counts" 
    cell_type_key: str | None = "cell_type",  # set None to skip GT proportions
):
    ...
    # incidence matrix B (n_bins x n_cells), B[b,c]=1 if cell c in bin b
    B = sp.csr_matrix(
    ...
    # synthetic spot expression = sum of member cells
    spot_X = B @ X  # stays sparse if X is sparse

    # spot coords = mean of member cell coords
    ...
    # ground-truth proportions per spot (from labels)
    if cell_type_key is not None and cell_type_key in adata_cells.obs:
        ...
        spot_ct = (B @ C).toarray()  # (n_spots, n_ct) counts per type
